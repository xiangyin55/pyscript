...
def uppois (f_specs,ad):
    ...
    wf = open('db\\pois.json','a',encoding='utf-8')
    wfs = open('db\\shape.json','a',encoding='utf-8')
    for l in open(f_specs,'r',encoding='utf-8'):
        ...
        try:
            sql = '''INSERT INTO pois (id,parent,name,x,y,city,adcode,type,star,tag,theme,score,shape,zoom,cover) VALUES ( ?, ?, ?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'''
        ...
        if j['shape'] :
            ...
            p['_id'] = j['id']
            p['shape'] = j['shape']
            p['rectangle'] = j['shape_rectangle']
            wfs.write(json.dumps(p,ensure_ascii = False)+"\n")
            try:
                sql = '''INSERT INTO shape (id,shape,rectangle) VALUES (  ?, ?, ?)'''
...
#    conn = sqlite3.connect(u'db\\bany.db')
    #conn = sqlite3.connect(u'E:\\APICloud\\bany\\res\\bany.db')
    cur = conn.cursor()
    wf = open('db\\intro.json','a',encoding='utf-8')
    ...
    for l in open(f_pick,'r',encoding='utf-8'):
        ...
        try:
            if s['pic']:
                sql = '''INSERT INTO intro (poiid,parent,stype,content) VALUES (  ?, ?, ?, ?)'''
                cur.execute(sql,(s['id'],s['parent'],'image',json.dumps(s['pic'],ensure_ascii=False)))
                o = {}
                o['_id'] = s['id']
                o['parent'] = s['parent']
                o['stype'] = 'image'
            ...
            if s['special']:
                sql = '''INSERT INTO intro (poiid,parent,stype,content) VALUES (  ?, ?, ?, ?)'''
                cur.execute(sql,(s['id'],s['parent'],'special',s['special']))
                o = {}
                o['_id'] = s['id']
                o['parent'] = s['parent']
                o['stype'] = 'special'
            ...
            if s['intro']:
                sql = '''INSERT INTO intro (poiid,parent,stype,content) VALUES (  ?, ?, ?, ?)'''
                cur.execute(sql,(s['id'],s['parent'],'intro',s['intro']))
                o = {}
                o['_id'] = s['id']
                o['parent'] = s['parent']
                o['stype'] = 'intro'
            ...
            if s['tts']:
                sql = '''INSERT INTO intro (poiid,parent,stype,content) VALUES (  ?, ?, ?, ?)'''
                cur.execute(sql,(s['id'],s['parent'],'guide',s['tts']))
                o = {}
                o['_id'] = s['id']
                o['parent'] = s['parent']
                o['stype'] = 'guide'
...
if __name__ == '__main__':
    ...
    for ad in adcodes:
        ...
        uppois("pick\\"+ad+"0000_spec.json",ad)
        upintro("pick\\"+ad+"0000_pickpoi.json")
